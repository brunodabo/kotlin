...
public abstract class AbstractReplInterpreterTest : UsefulTestCase() {
    ...
    protected fun doTest(path: String) {
        ...
        for ((code, expected, matchType) in loadLines(File(path))) {
            val expected = StringUtil.convertLineSeparators(expected).replaceFirst("\n$", "")

            val lineResult = repl.eval(code)
            val actual = when (lineResult.getType()) {
                ReplInterpreter.LineResultType.SUCCESS -> lineResult.getValue()
            ...
            val actualString = StringUtil.convertLineSeparators(actual?.toString() ?: "").replaceFirst("\n$", "")
            ...
            when (matchType) {
                ...
                MatchType.SUBSTRING -> {
                    Assert.assertTrue("Evaluated result must contain substring: $expected, actual: $actualString, line: $code",
                                      expected in actualString)
                }
            }
        }
    }
